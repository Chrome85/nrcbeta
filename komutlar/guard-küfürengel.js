...
exports.run = async(client, message) => {
  ...
  if(!message.member.roles.cache.has(yrol)) return message.channel.send(`Bu komutu kullanabilmek için YETKİLİ ROLÜNE sahip olmalısın.`)
	...
	if(secenekler.length < 1) return message.reply(`**n.küfür-engelle aç** veya **n.küfür-engelle kapat** yazınz `);
	//if(secenekler === "aç" || "kapat") return message.channel.send(errembed);

  if (secenekler !== "aç" && secenekler !== "kapat" && secenekler !== "on" && secenekler !== "off") return message.reply(`**n.küfür-engelle aç** veya **n.küfür-engelle kapat** yazınz `)
    ...
    .setColor('#f6ff00')
    .setDescription(`Küfür Engeli Başarıyla açıldı\nKüfür engelini kapatmak isterseniz **n.küfür-engel kapat** yazmanız yeterlidir.`)
